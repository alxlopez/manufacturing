...
@Component
public class WorkflowNodeRestartingState extends WorkflowNodeArtifactState {	
	@Override
	...
	public void complete(WorkflowNode workflowNode, ExecutionEvent event) {
		...
		.completeRoutine(workflowNode, event);
		workflowNode.getArtifact().getArtifactService()
		.setVariableLocal(workflowNode.getArtifactId(), BaseWorkflowNode.PROP_STATE, States.RUNNING.toString());
	}
}
