...
@Component
public abstract class WorkflowNodeArtifactRoutineStopping extends WorkflowNodeArtifactRoutine {
	@Override
	public void execute(WorkflowNode workflowNode, ExecutionEvent event) {
		...
		workflowNodeService.setState(workflowNode.getArtifactId(), States.STOPPING);
	}

	@Override
	public void completeRoutine(WorkflowNode workflowNode, ExecutionEvent event) {
		ArtifactService workflowNodeService= workflowNode.getArtifact().getArtifactService();
		workflowNodeService.setState(workflowNode.getArtifactId(), States.STOPPED);
	}
}
