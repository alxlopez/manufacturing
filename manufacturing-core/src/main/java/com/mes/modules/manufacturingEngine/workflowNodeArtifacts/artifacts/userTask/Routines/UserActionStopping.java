...
@Component
public class UserActionStopping extends WorkflowNodeArtifactRoutineStopping {
	@Autowired
	...
	public void execute(WorkflowNode workflowNode, ExecutionEvent event) {
		...
		String rutineId = (String) workflowNodeService.getVariableLocal(workflowNode.getArtifactId(), "rutineId");
		EventParameter eventComment = event.getParameter("comment");
		if (eventComment != null) {
			deleteReason = eventComment.getValue();
		}
		if (rutineId != null) {
			eventService.throwEvent(Long.valueOf(rutineId), EventEntities.PROCESS, Commands.STOP);
